...
local function handle_new_line_insert(command)
	...
	return table.concat({
		...
		command, "$<Esc>m`",                        -- insert a `$` marker for indentation
		total_lines_to_insert, opposite_command, "<Esc>", -- insert blank lines
		"g``\"_s",                                  -- return to the marker and start insert mode
	})
end

local function bind_new_line(key, command)
	vim.keymap.set('n', key, function() return handle_new_line_insert(command) end, { expr = true, silent = true })
end
...
M.set_keybindings = function()
	...
	-- rename
	bind("n", "<leader>rr", function() require("live-rename").rename() end)
end
